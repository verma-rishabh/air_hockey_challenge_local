...
class exp1_agent(AgentBase):
	...
	def train(self, replay_buffer, batch_size=256):
		...
		critic_loss = F.mse_loss(current_Q, target_Q,reduction='none').mean(axis=0)
		_critic_loss = critic_loss.cpu().data.numpy().mean()
		# Optimize the critic
		self.critic_optimizer.zero_grad()
		critic_loss.backward(torch.ones((14,)).to(device)*1.0)
		self.critic_optimizer.step()

		# Compute actor loss
		actor_loss = -self.critic(state, self.actor(state)).mean(axis=0)
		_actor_loss = actor_loss.cpu().data.numpy().mean()
		# Optimize the actor 
		self.actor_optimizer.zero_grad()
		actor_loss.backward(torch.ones((14,)).to(device)*1.0)
