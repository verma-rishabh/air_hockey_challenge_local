...
from air_hockey_agent.agent_builder_td3_defend import build_agent
...
def cust_rewards(policy,state,done,episode_timesteps):
    ...
    if (puck_pos[0]<0.580) and not wall_hit:
        reward -= 100
        wall_hit = 1
    # reward-=done*100
...
def main():
    ...
    parser.add_argument("--env", default="air-hockey-defend")          # OpenAI gym environment name
    ...
    main_dir = "/run/media/luke/Data/uni/SS2023/DL Lab/Project/TD3-Defend"
    ...
    env = AirHockeyChallengeWrapper(env="3dof-defend", action_type="position-velocity", interpolation_order=3, debug=False)
    ...
    max_ = np.stack([pos_max,vel_max]) # ee_pos = policy.get_ee_pose(state)[0]                               
    # puck_pos = policy.get_puck_pos(state)
    # dist = np.linalg.norm(ee_pos-puck_pos)
    ...
    for t in range(int(args.max_timesteps)):
        ...
        if t < args.start_timesteps:
            ...
            action = (
                ...
                + np.random.normal(0, max_action * args.expl_noise, size=action_dim*2)
            ).clip(-max_action, max_action)
          
        # Perform action
        next_state, reward, done, info_ = env.step(action.reshape(2,3)) 
        # print(next_state[3])
        # env.render()
